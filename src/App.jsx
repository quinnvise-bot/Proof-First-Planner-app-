...
const ProofFirstPlanner = () => {
  ...
  const [newProofEntry, setNewProofEntry] = useState({ date: new Date().toISOString().split('T')[0], serviceTotal: '', tips: '', hoursWorked: '', commissionRate: '' });
  ...
  const Input = ({ label, ...props }) => (
    <div style={{ minWidth: 0 }}>
      ...
      <input {...props} style={{ width: '100%', maxWidth: '100%', padding: '12px 14px', border: '1px solid #E8E0D8', borderRadius: 8, fontSize: 14, fontFamily: "'Montserrat', sans-serif", outline: 'none', background: '#FDFCFB', transition: 'border-color 0.2s', color: '#2D2D2D', boxSizing: 'border-box', minWidth: 0, ...props.style }} onFocus={e => e.target.style.borderColor = '#D4AF37'} onBlur={e => e.target.style.borderColor = '#E8E0D8'} />
    </div>
  );
              ...
              {showAddEntry && (
                <div style={{ 
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
                    <Input label="Date" type="date" value={newProofEntry.date} onChange={e => setNewProofEntry({...newProofEntry, date: e.target.value})} />
                  ...
                  <div style={{ marginBottom: 16 }}>
                    <Input label="Commission Rate (%)" type="number" placeholder="45" value={newProofEntry.commissionRate} onChange={e => setNewProofEntry({...newProofEntry, commissionRate: e.target.value})} />
                  </div>
                  ...
                  {newProofEntry.serviceTotal && newProofEntry.hoursWorked && (
                    <div style={{ 
                      ...
                      <p style={{ fontSize: 24, fontWeight: 700, color: '#D4AF37', margin: '4px 0 0', fontFamily: "'Cormorant Garamond', Georgia, serif" }}>
                        ${(((parseFloat(newProofEntry.serviceTotal) || 0) * ((parseFloat(newProofEntry.commissionRate) || 45) / 100) + (parseFloat(newProofEntry.tips) || 0)) / (parseFloat(newProofEntry.hoursWorked) || 1)).toFixed(0)}/hr
                      </p>
                    </div>
                  )}
                  ...
                  <Btn variant="gold" onClick={() => {
                    if (newProofEntry.serviceTotal && newProofEntry.hoursWorked) {
                      setProofEntries([...proofEntries, { ...newProofEntry, id: Date.now() }]);
                      setNewProofEntry({ date: new Date().toISOString().split('T')[0], serviceTotal: '', tips: '', hoursWorked: '', commissionRate: newProofEntry.commissionRate || '45' });
