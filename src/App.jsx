...
const Input = ({ label, ...props }) => (
  <div style={{ minWidth: 0 }}>
    ...
    <input {...props} style={{ width: '100%', maxWidth: '100%', padding: '12px 14px', border: '1px solid #E8E0D8', borderRadius: 8, fontSize: 16, fontFamily: "'Montserrat', sans-serif", outline: 'none', background: '#FDFCFB', color: '#2D2D2D', boxSizing: 'border-box', minWidth: 0, ...props.style }} />
  </div>
);
...
const Select = ({ label, children, ...props }) => (
  <div>
    {label && <label style={{ display: 'block', fontSize: 10, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#8B7D6B', marginBottom: 6, fontFamily: "'Montserrat', sans-serif", fontWeight: 500 }}>{label}</label>}
    <select {...props} style={{ width: '100%', padding: '12px 16px', border: '1px solid #E8E0D8', borderRadius: 8, fontSize: 16, fontFamily: "'Montserrat', sans-serif", outline: 'none', background: '#FDFCFB', color: '#2D2D2D', boxSizing: 'border-box', ...props.style }}>{children}</select>
  </div>
);
...
const ProofFirstPlanner = () => {
  ...
  const [calculatorData, setCalculatorData] = useState({ servicePrice: '', timeMinutes: '', commissionRate: '', tips: '' });
  const [proofPeriod, setProofPeriod] = useState('all');
  const [proofEntries, setProofEntries] = useState([]);
  const [newProofEntry, setNewProofEntry] = useState({ date: new Date().toISOString().split('T')[0], serviceTotal: '', tips: '', hoursWorked: '', commissionRate: '', rebooks: '', retailSales: '' });
  const [showAddEntry, setShowAddEntry] = useState(false);
  const [showExtras, setShowExtras] = useState(false);
  const [pillars, setPillars] = useState({
    purpose: { clientTransformation: '', whyBehindChair: '', dreamLegacy: '' },
    progress: { currentlyLearning: '', skillToMaster: '', educationGoal: '' },
    profit: { monthlyIncomeGoal: '', biggestRevenueBlock: '', nextPricingMove: '' },
    process: { consultationApproach: '', timingStrategy: '', systemToImprove: '' }
  });
    ...
    const loadAllData = async () => {
      ...
      try {
        const [
          ...
          savedMyChallenges, savedMilestones, savedEntries, savedCommission, savedProofEntries
        ] = await Promise.all([
        ...
        if (savedProofEntries) setProofEntries(savedProofEntries);
      } catch (error) {
  ...
  useEffect(() => { if (userName) saveData('proofEntries', proofEntries); }, [proofEntries, userName]);
  ...
  const getCurrentStats = () => {
    ...
    const avgTicket = totalClients > 0 ? totalServiceSales / totalClients : 0;
  ...
  const pillarConfig = {
    purpose: { label: 'Purpose', num: '01', color: '#C9B8A8', desc: 'Your WHY — the fire that keeps you behind the chair.' },
    progress: { label: 'Progress', num: '02', color: '#D4AF37', desc: 'Your growth edge — what you\'re learning and mastering.' },
    profit: { label: 'Profit', num: '03', color: '#D4A5A5', desc: 'Your money strategy — income goals, pricing, and revenue.' },
    process: { label: 'Process', num: '04', color: '#8B7D6B', desc: 'Your systems — how you run your chair like a business.' }
  };

  const pillarLabels = {
    clientTransformation: 'How do you ensure your guests leave confident and ready to win their week?',
    whyBehindChair: 'Why do you show up behind the chair every day?',
    dreamLegacy: 'What do you want to be known for in 5 years?',
    currentlyLearning: 'What skill or technique are you currently learning?',
    skillToMaster: 'What\'s the next thing you want to master?',
    educationGoal: 'What education or certification are you working toward?',
    monthlyIncomeGoal: 'What\'s your monthly take-home income goal?',
    biggestRevenueBlock: 'What\'s the biggest thing holding back your revenue?',
    nextPricingMove: 'What\'s your next pricing or service menu move?',
    consultationApproach: 'How do you start every client consultation?',
    timingStrategy: 'How do you manage your timing to stay on schedule?',
    systemToImprove: 'What system in your business needs the most work?'
  };
  ...
  const tabs = [
    ...
    { id: 'progress', label: 'My Level', hasBolt: true },
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(175deg, #FAF7F2 0%, #F5F0E8 40%, #FAF7F2 70%, #F2EDE5 100%)', fontFamily: "'Montserrat', sans-serif" }}>
        ...
        <div style={{ maxWidth: 520, margin: '0 auto', padding: '8px 12px', display: 'flex', gap: 4, overflowX: 'auto' }}>
          {tabs.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
              ...
              {tab.hasBolt && <svg width="12" height="12" viewBox="0 0 24 24" fill={activeTab === tab.id ? '#D4AF37' : '#D4AF37'}><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>}
              {tab.label}
            </button>
          ))}
        </div>
      ...
      <main style={{ maxWidth: 520, margin: '0 auto', padding: '24px 16px 40px' }}>
            ...
            {progressView === 'map' && (
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 20 }}>
                  ...
                  <Card style={{ padding: 16, textAlign: 'center' }}>
                    <p style={{ fontSize: 9, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#8B7D6B', marginBottom: 4 }}>Avg Ticket</p>
                    <p style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 28, fontWeight: 300, color: '#2D2D2D', margin: 0 }}>${stats.avgTicket}</p>
                  </Card>
                  <Card style={{ padding: 16, textAlign: 'center' }}>
        ...
        {/* ═══════════════ THE PROOF - RACE CAR DASHBOARD ═══════════════ */}
        {activeTab === 'proof' && (
          <div>
            <SectionHead eyebrow="Your Race Car" title="The" accent="Proof" subtitle="Build a career with facts, not feelings. Proof, not highlight reels." />
            
            {/* Race Car Speedometer - Full Width, No Box */}
            <div style={{ marginBottom: 12 }}>
              {/* Speedometer Container - Full Width */}
              <div style={{ 
                width: '100%',
                margin: '0 auto',
                position: 'relative'
                ...
                {/* Gauge Background */}
                <svg viewBox="0 0 280 165" style={{ width: '100%', height: 'auto' }} preserveAspectRatio="xMidYMid meet">
                  {/* Outer ring - cream/gold */}
                  <path d="M 20 145 A 120 120 0 0 1 260 145" fill="none" stroke="#E8E0D8" strokeWidth="6" />
                  {/* Inner track - subtle */}
                  <path d="M 30 145 A 110 110 0 0 1 250 145" fill="none" stroke="#F5F0E8" strokeWidth="20" />
                  {/* Gold accent arc */}
                  <path d="M 30 145 A 110 110 0 0 1 250 145" fill="none" stroke="url(#gaugeGradient)" strokeWidth="3" />
                  ...
                  {[0, 50, 100, 150, 200, 250].map((value, i) => {
                    const angle = -180 + (value / 250) * 180;
                    const radian = (angle * Math.PI) / 180;
                    const x1 = 140 + 95 * Math.cos(radian);
                    const y1 = 145 + 95 * Math.sin(radian);
                    const x2 = 140 + 112 * Math.cos(radian);
                    const y2 = 145 + 112 * Math.sin(radian);
                    const textX = 140 + 78 * Math.cos(radian);
                    const textY = 145 + 78 * Math.sin(radian);
                    return (
                      <g key={value}>
                        <line x1={x1} y1={y1} x2={x2} y2={y2} stroke="#D4AF37" strokeWidth="2.5" />
                        <text x={textX} y={textY} fill="#8B7D6B" fontSize="12" fontFamily="Montserrat, sans-serif" fontWeight="600" textAnchor="middle" dominantBaseline="middle">{value}</text>
                      </g>
                    );
                  })}
                  ...
                  {[...Array(26)].map((_, i) => {
                    const value = i * 10;
                    if (value % 50 === 0) return null;
                    const angle = -180 + (value / 250) * 180;
                    const radian = (angle * Math.PI) / 180;
                    const x1 = 140 + 100 * Math.cos(radian);
                    const y1 = 145 + 100 * Math.sin(radian);
                    const x2 = 140 + 108 * Math.cos(radian);
                    const y2 = 145 + 108 * Math.sin(radian);
                    return <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="#C9B8A8" strokeWidth="1.5" />;
                  })}
                  
                  {/* Needle */}
                  {(() => {
                    const filteredEntries = proofEntries.length > 0 ? (() => {
                      const now = new Date();
                      ...
                      switch(proofPeriod) {
                        case 'day':
                          return proofEntries.filter(e => e.date === now.toISOString().split('T')[0]);
                        case 'week':
                          const weekAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
                          return proofEntries.filter(e => new Date(e.date) >= weekAgo);
                        case 'payperiod':
                          const periodStart = currentDay <= 15 ? new Date(currentYear, currentMonth, 1) : new Date(currentYear, currentMonth, 16);
                          return proofEntries.filter(e => new Date(e.date) >= periodStart);
                        case 'month':
                          return proofEntries.filter(e => {
                            const d = new Date(e.date);
                            return d.getMonth() === currentMonth && d.getFullYear() === currentYear;
                          });
                        case 'quarter':
                          const quarterStart = new Date(currentYear, Math.floor(currentMonth / 3) * 3, 1);
                          return proofEntries.filter(e => new Date(e.date) >= quarterStart);
                        case 'year':
                          return proofEntries.filter(e => new Date(e.date).getFullYear() === currentYear);
                        default:
                          return proofEntries;
                      }
                    })() : [];
                    
                    const totalEarnings = filteredEntries.reduce((sum, e) => {
                      const serviceEarnings = (parseFloat(e.serviceTotal) || 0) * ((parseFloat(e.commissionRate) || 45) / 100);
                      return sum + serviceEarnings + (parseFloat(e.tips) || 0);
                    }, 0);
                    const totalHours = filteredEntries.reduce((sum, e) => sum + (parseFloat(e.hoursWorked) || 0), 0);
                    const avgHourlyRate = totalHours > 0 ? totalEarnings / totalHours : 0;
                    const clampedRate = Math.min(250, Math.max(0, avgHourlyRate));
                    const needleAngle = -180 + (clampedRate / 250) * 180;
                    const needleRadian = (needleAngle * Math.PI) / 180;
                    const needleLength = 85;
                    const needleX = 140 + needleLength * Math.cos(needleRadian);
                    const needleY = 145 + needleLength * Math.sin(needleRadian);
                    
                    return (
                      <g>
                        {/* Needle shadow */}
                        <line x1="140" y1="147" x2={needleX + 1} y2={needleY + 1} stroke="rgba(0,0,0,0.15)" strokeWidth="4" strokeLinecap="round" />
                        {/* Needle */}
                        <line x1="140" y1="145" x2={needleX} y2={needleY} stroke="#D4AF37" strokeWidth="3" strokeLinecap="round" />
                        {/* Center cap */}
                        <circle cx="140" cy="145" r="10" fill="#2D2D2D" stroke="#D4AF37" strokeWidth="2" />
                        <circle cx="140" cy="145" r="4" fill="#D4AF37" />
                      </g>
                    );
                  })()}
                  ...
                  {(() => {
                    ...
                    return (
                      <g>
                        <text x="140" y="115" fill="#D4AF37" fontSize="28" fontFamily="Cormorant Garamond, Georgia, serif" fontWeight="600" textAnchor="middle">${avgHourlyRate.toFixed(0)}</text>
                        <text x="140" y="130" fill="#8B7D6B" fontSize="8" fontFamily="Montserrat, sans-serif" fontWeight="500" textAnchor="middle" letterSpacing="0.1em">AVG PER HOUR</text>
                      </g>
                    );
                  })()}
                  
                  {/* Branding */}
                  <text x="140" y="158" fill="#D4AF37" fontSize="6" fontFamily="Montserrat, sans-serif" fontWeight="600" textAnchor="middle" letterSpacing="0.3em">PROOF FIRST FOUNDER™</text>
                </svg>
                
                {/* Last Updated - below gauge */}
                {proofEntries.length > 0 && (
                  <p style={{ 
                    ...
                    margin: '8px 0 0',
                    fontFamily: "'Montserrat', sans-serif"
                  }}>Last entry: {proofEntries[proofEntries.length - 1]?.date}</p>
                )}

                {/* Annualized Earnings Display */}
                {(() => {
                  const allEntries = proofEntries;
                  if (allEntries.length === 0) return null;
                  const totalEarnings = allEntries.reduce((sum, e) => {
                    const serviceEarnings = (parseFloat(e.serviceTotal) || 0) * ((parseFloat(e.commissionRate) || parseFloat(stylistCommission) || 41) / 100);
                    return sum + serviceEarnings + (parseFloat(e.tips) || 0);
                  }, 0);
                  ...
                  const annualized = hourlyRate * 32 * 50;
                  ...
                  const periodEnd = currentDay <= 15 ? 15 : lastDayOfMonth;
                  const daysLeft = periodEnd - currentDay;
                  const periodStart = currentDay <= 15 ? new Date(currentYear, currentMonth, 1) : new Date(currentYear, currentMonth, 16);
                  const periodEntries = proofEntries.filter(e => new Date(e.date) >= periodStart);
                  const periodServiceTotal = periodEntries.reduce((sum, e) => sum + (parseFloat(e.serviceTotal) || 0), 0);
                  const periodCommRate = (parseFloat(stylistCommission) || qvdLevels[stats.level - 1]?.commission || 41) / 100;
                  const periodEarned = periodEntries.reduce((sum, e) => {
                    return sum + (parseFloat(e.serviceTotal) || 0) * ((parseFloat(e.commissionRate) || parseFloat(stylistCommission) || 41) / 100) + (parseFloat(e.tips) || 0);
                  }, 0);
                  
                  return (
                    <div style={{ textAlign: 'center', marginTop: 12 }}>
                      <p style={{ fontSize: 14, color: '#D4AF37', fontWeight: 600, margin: '0 0 4px', fontFamily: "'Cormorant Garamond', Georgia, serif" }}>
                        ◆ ${hourlyRate.toFixed(0)}/hr
                      </p>
                      {hourlyRate > 0 && (
                        <p style={{ fontSize: 11, color: '#8B7D6B', margin: '0 0 8px', fontFamily: "'Montserrat', sans-serif" }}>
                          At this pace: ${Math.round(annualized).toLocaleString()}/year · QVD full-time (32 hrs × 50 wks)
                        </p>
                      )}
                      {daysLeft <= 8 && periodEntries.length > 0 && (
                        <div style={{ 
                          ...
                          <p style={{ fontSize: 11, color: '#2D2D2D', margin: 0, fontFamily: "'Montserrat', sans-serif" }}>
                            You've built <span style={{ fontWeight: 700, color: '#D4AF37' }}>${Math.round(periodEarned).toLocaleString()}</span> this pay period · {daysLeft} day{daysLeft !== 1 ? 's' : ''} left
                          </p>
                        </div>
                      )}
                    </div>
            ...
            <div style={{ 
              ...
              {[
                ...
                <button 
                  key={period.id}
                  onClick={() => setProofPeriod(period.id)}
                  style={{
                    padding: '8px 12px',
                    borderRadius: 20,
                    fontSize: 10,
                    fontWeight: proofPeriod === period.id ? 700 : 500,
                    letterSpacing: '0.05em',
                    border: 'none',
                    cursor: 'pointer',
                    background: proofPeriod === period.id ? '#2D2D2D' : '#FFFFFF',
                    color: proofPeriod === period.id ? '#FAF7F2' : '#8B7D6B',
                    fontFamily: "'Montserrat', sans-serif",
            ...
            <Card style={{ marginBottom: 20 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: showAddEntry ? 16 : 0 }}>
                <div>
                  <p style={{ fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#D4AF37', fontWeight: 600, margin: 0 }}>Your Proof Log</p>
                  <p style={{ fontSize: 11, color: '#8B7D6B', margin: '4px 0 0', fontFamily: "'Cormorant Garamond', Georgia, serif", fontStyle: 'italic' }}>Measure the proof.</p>
                </div>
                <button 
                  onClick={() => setShowAddEntry(!showAddEntry)}
                  style={{
                    background: showAddEntry ? '#8B7D6B' : '#D4AF37',
                    color: '#FFFFFF',
                    border: 'none',
                    borderRadius: 20,
                    padding: '8px 16px',
                ...
                >{showAddEntry ? 'Cancel' : '+ Add Proof'}</button>
              </div>
              ...
              {showAddEntry && (
                <div style={{ 
                  ...
                  {/* Date - own row, centered */}
                  <div style={{ marginBottom: 16 }}>
                    <Input label="Date" type="date" value={newProofEntry.date} onChange={e => setNewProofEntry({...newProofEntry, date: e.target.value})} style={{ textAlign: 'center', WebkitAppearance: 'none', MozAppearance: 'none', appearance: 'none' }} />
                  </div>
                  {/* Row 1: Service Total | Hours Worked */}
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
                    <Input label="Service Total ($)" type="number" placeholder="450" value={newProofEntry.serviceTotal} onChange={e => setNewProofEntry({...newProofEntry, serviceTotal: e.target.value})} />
                    <Input label="Hours Worked" type="number" placeholder="8" value={newProofEntry.hoursWorked} onChange={e => setNewProofEntry({...newProofEntry, hoursWorked: e.target.value})} />
                  </div>
                  {/* Row 2: Rebooks Secured */}
                  <div style={{ marginBottom: 16 }}>
                    <Input label="Rebooks Secured" type="number" placeholder="4" value={newProofEntry.rebooks} onChange={e => setNewProofEntry({...newProofEntry, rebooks: e.target.value})} />
                  </div>
                  {/* Expandable: Tips & Retail */}
                  <button onClick={() => setShowExtras(!showExtras)} style={{
                    background: 'none', border: 'none', padding: '8px 0', fontSize: 11, color: '#D4AF37',
                    fontWeight: 600, cursor: 'pointer', fontFamily: "'Montserrat', sans-serif", letterSpacing: '0.05em',
                    display: 'flex', alignItems: 'center', gap: 6, marginBottom: showExtras ? 12 : 0
                  }}>
                    <span style={{ transform: showExtras ? 'rotate(90deg)' : 'rotate(0deg)', transition: 'transform 0.2s', display: 'inline-block' }}>▸</span>
                    {showExtras ? 'Hide Tips & Retail' : '+ Add Tips & Retail'}
                  </button>
                  {showExtras && (
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
                      <Input label="Tips ($)" type="number" placeholder="85" value={newProofEntry.tips} onChange={e => setNewProofEntry({...newProofEntry, tips: e.target.value})} />
                      <Input label="Retail $ Sold" type="number" placeholder="45" value={newProofEntry.retailSales} onChange={e => setNewProofEntry({...newProofEntry, retailSales: e.target.value})} />
                    </div>
                  )}
                  {/* Commission auto-filled from level - hidden but used in calc */}
                  
                  {/* Live Preview */}
                  {newProofEntry.serviceTotal && newProofEntry.hoursWorked && (
                    <div style={{ 
                      ...
                      <p style={{ fontSize: 9, color: '#8B7D6B', margin: 0, letterSpacing: '0.1em', textTransform: 'uppercase' }}>This Entry's Rate</p>
                      <p style={{ fontSize: 24, fontWeight: 700, color: '#D4AF37', margin: '4px 0 0', fontFamily: "'Cormorant Garamond', Georgia, serif" }}>
                        ${(((parseFloat(newProofEntry.serviceTotal) || 0) * ((parseFloat(newProofEntry.commissionRate) || parseFloat(stylistCommission) || qvdLevels[stats.level - 1]?.commission || 41) / 100) + (parseFloat(newProofEntry.tips) || 0)) / (parseFloat(newProofEntry.hoursWorked) || 1)).toFixed(0)}/hr
                      </p>
                    </div>
                  )}
                  
                  <Btn variant="gold" onClick={() => {
                    if (newProofEntry.serviceTotal && newProofEntry.hoursWorked) {
                      const autoCommission = newProofEntry.commissionRate || stylistCommission || String(qvdLevels[stats.level - 1]?.commission || 41);
                      setProofEntries([...proofEntries, { ...newProofEntry, commissionRate: autoCommission, id: Date.now() }]);
                      setNewProofEntry({ date: new Date().toISOString().split('T')[0], serviceTotal: '', tips: '', hoursWorked: '', commissionRate: '', rebooks: '', retailSales: '' });
              ...
              {/* Entry List - Checkbook Style */}
              {proofEntries.length > 0 ? (
                <div style={{ marginTop: showAddEntry ? 0 : 16 }}>
                  <div style={{ 
                    display: 'grid', 
                  ...
                  {[...proofEntries].reverse().slice(0, 10).map((entry, i) => {
                    const hourlyRate = ((parseFloat(entry.serviceTotal) || 0) * ((parseFloat(entry.commissionRate) || 45) / 100) + (parseFloat(entry.tips) || 0)) / (parseFloat(entry.hoursWorked) || 1);
                    return (
                      <div 
                        key={entry.id} 
                        style={{ 
                          ...
                          gap: 8,
                          padding: '12px',
                          background: i % 2 === 0 ? '#FDFCFB' : '#F5F0E8',
                          fontSize: 12,
                          color: '#2D2D2D',
                      ...
                      >
                        <span style={{ fontSize: 11, color: '#8B7D6B' }}>{new Date(entry.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                        <span style={{ fontWeight: 600 }}>${entry.serviceTotal}</span>
                        <span style={{ color: '#D4AF37' }}>+${entry.tips || 0}</span>
                        <span>{entry.hoursWorked}h</span>
                        <span style={{ textAlign: 'right', fontWeight: 700, color: '#2D2D2D' }}>${hourlyRate.toFixed(0)}</span>
                      </div>
                    );
                  })}
                  {proofEntries.length > 10 && (
                    <p style={{ textAlign: 'center', fontSize: 11, color: '#8B7D6B', padding: 12, margin: 0 }}>Showing last 10 of {proofEntries.length} entries</p>
                  )}
                </div>
              ) : (
                <div style={{ textAlign: 'center', padding: 32 }}>
                  <BoltIcon size={32} fill="#D4AF37" />
                  <p style={{ fontSize: 14, color: '#8B7D6B', marginTop: 12, fontFamily: "'Cormorant Garamond', Georgia, serif", fontStyle: 'italic' }}>Are you race ready? Log your first lap.</p>
                </div>
              )}
            </Card>
            ...
            {/* Today's Challenge */}
            {(() => {
              const nextDay = challengePrompts.find(p => !completedDays.includes(p.day));
              if (!nextDay) return null;
              return (
                <Card style={{ marginBottom: 20, border: '1px solid rgba(212,175,55,0.15)' }}>
                  <p style={{ fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#D4AF37', fontWeight: 600, margin: '0 0 12px' }}>Today's Challenge</p>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <button onClick={() => toggleChallengeDay(nextDay.day)} style={{
                      width: 36, height: 36, borderRadius: '50%', border: `2px solid ${completedDays.includes(nextDay.day) ? '#7B9E87' : '#D4AF37'}`,
                      background: completedDays.includes(nextDay.day) ? '#7B9E87' : 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0
                    }}>
                      {completedDays.includes(nextDay.day) ? (
                        <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="#FFF" strokeWidth="3"><path d="M20 6L9 17l-5-5"/></svg>
                      ) : (
                        <span style={{ fontSize: 12, fontWeight: 700, color: '#D4AF37' }}>{nextDay.day}</span>
                      )}
                    </button>
                    <div style={{ flex: 1 }}>
                      <div style={{ marginBottom: 4 }}>
                        <span style={{ fontSize: 9, padding: '2px 8px', background: 'rgba(212,175,55,0.1)', borderRadius: 10, color: '#D4AF37', fontWeight: 600, letterSpacing: '0.05em' }}>{nextDay.type}</span>
                      </div>
                      <p style={{ fontSize: 13, color: '#2D2D2D', margin: 0, lineHeight: 1.4 }}>{nextDay.prompt}</p>
                    </div>
                  </div>
                </Card>
              );
            ...
            {/* Smart Coach — Phase 1 */}
            {(() => {
              if (proofEntries.length < 2) return null;
              const commRate = (parseFloat(stylistCommission) || qvdLevels[stats.level - 1]?.commission || 41) / 100;
              const now = new Date();
              const currentDay = now.getDate();
              const currentMonth = now.getMonth();
              const currentYear = now.getFullYear();
              ...
              const periodServiceTotal = periodEntries.reduce((s, e) => s + (parseFloat(e.serviceTotal) || 0), 0);
              const periodEarned = periodEntries.reduce((s, e) => {
                return s + (parseFloat(e.serviceTotal) || 0) * ((parseFloat(e.commissionRate) || parseFloat(stylistCommission) || 41) / 100) + (parseFloat(e.tips) || 0);
              }, 0);
              
              // Average per working day this period
              const workingDaysLogged = periodEntries.length;
              const avgDailyService = workingDaysLogged > 0 ? periodServiceTotal / workingDaysLogged : 0;
              const avgDailyEarned = workingDaysLogged > 0 ? periodEarned / workingDaysLogged : 0;
              
              // Project remaining days
              const projectedTotal = periodEarned + (avgDailyEarned * Math.min(daysLeft, 5));
              
              // Level target for context
              const levelTarget = qvdLevels[stats.level - 1]?.monthlyPotential || 3652;
              const halfMonthTarget = levelTarget / 2;
              const gap = halfMonthTarget - periodEarned;
              
              // Build coaching message
              let coachMessage = '';
              let coachTone = 'neutral'; // 'winning', 'close', 'push'
              
              if (periodEarned >= halfMonthTarget) {
                coachTone = 'winning';
                coachMessage = `You've already hit your Level ${stats.level} pace for this pay period. Keep this momentum — you're proving it.`;
              } else if (gap > 0 && gap < avgDailyEarned * 3) {
                coachTone = 'close';
                const extraDays = Math.ceil(gap / avgDailyEarned);
                coachMessage = `You're $${Math.round(gap).toLocaleString()} away from your Level ${stats.level} pace. At your average, that's about ${extraDays} more working day${extraDays !== 1 ? 's' : ''}. You've got this.`;
              } else if (gap > 0 && daysLeft > 0 && avgDailyService > 0) {
                coachTone = 'push';
                const neededPerDay = gap / Math.min(daysLeft, 5);
                coachMessage = `To close the gap this period, aim for ~$${Math.round(neededPerDay).toLocaleString()}/day in earnings over the next ${Math.min(daysLeft, 5)} days. Consider adding an extra hour or rebooking a client who's due.`;
              } else if (workingDaysLogged > 0) {
                coachMessage = `You're averaging $${Math.round(avgDailyEarned)}/day earned this period across ${workingDaysLogged} days logged. Keep tracking — the proof builds over time.`;
              }
              
              if (!coachMessage) return null;
              
              return (
                <Card style={{ marginBottom: 20, borderLeft: `3px solid ${coachTone === 'winning' ? '#7B9E87' : coachTone === 'close' ? '#D4AF37' : '#C9B8A8'}` }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                    <BoltIcon size={18} fill={coachTone === 'winning' ? '#7B9E87' : '#D4AF37'} />
                    <div>
                      <p style={{ fontSize: 10, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#D4AF37', fontWeight: 600, margin: '0 0 6px' }}>Your Coach</p>
                      <p style={{ fontSize: 13, color: '#2D2D2D', margin: 0, lineHeight: 1.6, fontFamily: "'Cormorant Garamond', Georgia, serif" }}>{coachMessage}</p>
                    </div>
                  </div>
                </Card>
              );
            ...
            {/* Quick Calculator */}
            <Card>
              <p style={{ fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#D4AF37', fontWeight: 600, marginBottom: 8 }}>Quick Calculator</p>
              <p style={{ fontSize: 12, color: '#2D2D2D', margin: '0 0 4px', fontFamily: "'Cormorant Garamond', Georgia, serif", lineHeight: 1.5 }}>Making the most of your work shifts requires a plan. Know the timing that gives the best result & rate per hour.</p>
              <p style={{ fontSize: 11, color: '#8B7D6B', margin: '0 0 16px', fontFamily: "'Montserrat', sans-serif", fontWeight: 600, fontStyle: 'italic' }}>This is a career, not a hobby. I'm in it to win it.</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  ...
                  <Input label="Service Time (min)" type="number" value={calculatorData.timeMinutes} onChange={e => setCalculatorData({...calculatorData, timeMinutes: e.target.value})} placeholder="90" />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  <Input label="Commission (%)" type="number" value={calculatorData.commissionRate} onChange={e => setCalculatorData({...calculatorData, commissionRate: e.target.value})} placeholder="45" />
                  <Input label="Tips ($)" type="number" value={calculatorData.tips} onChange={e => setCalculatorData({...calculatorData, tips: e.target.value})} placeholder="25" />
                </div>
              </div>
              {/* Result Display - Single Box */}
              <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid #EDE8E2' }}>
                <div style={{ background: 'linear-gradient(135deg, rgba(212,175,55,0.08), rgba(212,175,55,0.15))', borderRadius: 10, padding: 20, textAlign: 'center', border: '1px solid rgba(212,175,55,0.2)' }}>
                  <p style={{ fontSize: 9, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#D4AF37', marginBottom: 6, fontWeight: 600 }}>Your Hourly Rate</p>
                  <p style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 36, fontWeight: 600, color: '#2D2D2D', margin: 0 }}>
                    ${calculatorData.servicePrice && calculatorData.timeMinutes ? ((((parseFloat(calculatorData.servicePrice) || 0) * ((parseFloat(calculatorData.commissionRate) || 45) / 100)) + (parseFloat(calculatorData.tips) || 0)) / (parseFloat(calculatorData.timeMinutes) || 1) * 60).toFixed(0) : '—'}
                  </p>
                  <p style={{ fontSize: 10, color: '#8B7D6B', marginTop: 4, fontFamily: "'Montserrat', sans-serif" }}>per hour</p>
                </div>
              </div>
            </Card>
          </div>
        ...
        {/* ═══════════════ (OLD CALCULATOR REMOVED - NOW IN THE PROOF) ═══════════════ */}
              ...
              <Card key={key} style={{ marginBottom: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: 11, color: '#8B7D6B', margin: '4px 0 0', fontFamily: "'Cormorant Garamond', Georgia, serif", fontStyle: 'italic' }}>{config.desc}</p>
                  </div>
                </div>
                {/* Live Data Card for Profit */}
                {key === 'profit' && proofEntries.length > 0 && (() => {
                  const commRate = (parseFloat(stylistCommission) || qvdLevels[stats.level - 1]?.commission || 41) / 100;
                  const totalService = proofEntries.reduce((s, e) => s + (parseFloat(e.serviceTotal) || 0), 0);
                  const totalCommEarned = proofEntries.reduce((s, e) => s + (parseFloat(e.serviceTotal) || 0) * ((parseFloat(e.commissionRate) || parseFloat(stylistCommission) || 41) / 100), 0);
                  const totalRetail = proofEntries.reduce((s, e) => s + (parseFloat(e.retailSales) || 0), 0);
                  return (
                    <div style={{ background: '#FDFCF7', borderLeft: '3px solid #D4AF37', borderRadius: '0 8px 8px 0', padding: '12px 16px', marginBottom: 16 }}>
                      <p style={{ fontSize: 9, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#D4AF37', fontWeight: 600, margin: '0 0 8px' }}>Your Numbers</p>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#2D2D2D', marginBottom: 4 }}>
                        <span>Service total</span><span style={{ fontWeight: 600 }}>${Math.round(totalService).toLocaleString()}</span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#2D2D2D', marginBottom: 4 }}>
                        <span>Commission earned ({Math.round(commRate * 100)}%)</span><span style={{ fontWeight: 600 }}>${Math.round(totalCommEarned).toLocaleString()}</span>
                      </div>
                      {totalRetail > 0 && (
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#2D2D2D' }}>
                          <span>Retail sold</span><span style={{ fontWeight: 600 }}>${Math.round(totalRetail).toLocaleString()}</span>
                        </div>
                      )}
                    </div>
                  );
                })()}
                {/* Live Data Card for Progress */}
                {key === 'progress' && (proofEntries.length > 0 || completedDays.length > 0) && (() => {
                  const totalRebooks = proofEntries.reduce((s, e) => s + (parseFloat(e.rebooks) || 0), 0);
                  return (
                    <div style={{ background: '#FDFCF7', borderLeft: '3px solid #D4AF37', borderRadius: '0 8px 8px 0', padding: '12px 16px', marginBottom: 16 }}>
                      <p style={{ fontSize: 9, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#D4AF37', fontWeight: 600, margin: '0 0 8px' }}>Your Progress</p>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#2D2D2D', marginBottom: 4 }}>
                        <span>Challenge days completed</span><span style={{ fontWeight: 600 }}>{completedDays.length}/30</span>
                      </div>
                      {totalRebooks > 0 && (
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#2D2D2D' }}>
                          <span>Rebooks logged</span><span style={{ fontWeight: 600 }}>{totalRebooks}</span>
                        </div>
                      )}
                    </div>
                  );
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {Object.entries(pillars[key]).map(([field, value]) => (
                    <Input key={field} label={pillarLabels[field] || field.replace(/([A-Z])/g, ' $1').trim()} value={value} onChange={e => setPillars({...pillars, [key]: {...pillars[key], [field]: e.target.value}})} placeholder="Your answer..." />
                  ))}
                </div>
              </Card>
            ))}

            {/* Social Visibility — moved from standalone tab */}
            <div style={{ marginTop: 16 }}>
              <SectionHead eyebrow="Off-Chair Revenue" title="Social" accent="Visibility" subtitle="Social visibility is your off-chair revenue engine — tracked at Level 5+." />
              <Card>
        ...
        {activeTab === 'mygoals' && (
          <div>
            ...
            <div style={{ marginTop: 32 }}>
              <SectionHead eyebrow="Proof of Process" title="My" accent="Milestones" subtitle="Milestones that map your journey." />
              <Card style={{ marginBottom: 20 }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div>
            ...
            <Card style={{ overflow: 'visible' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, padding: '0 4px' }}>
                <button onClick={() => { if (currentMonth === 0) { setCurrentMonth(11); setCurrentYear(currentYear - 1); } else setCurrentMonth(currentMonth - 1); }} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 18, color: '#8B7D6B', padding: 8 }}>←</button>
                ...
                <button onClick={() => { if (currentMonth === 11) { setCurrentMonth(0); setCurrentYear(currentYear + 1); } else setCurrentMonth(currentMonth + 1); }} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 18, color: '#8B7D6B', padding: 8 }}>→</button>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 2, marginBottom: 16, width: '100%', minWidth: 0 }}>
                {dayNames.map((d, i) => <div key={i} style={{ textAlign: 'center', fontSize: 9, fontWeight: 600, color: '#B0A698', padding: 4 }}>{d}</div>)}
                ...
                {Array.from({ length: getDaysInMonth(currentYear, currentMonth) }, (_, i) => {
                  ...
                  return (
                    <button key={day} onClick={() => setSelectedDate(day === selectedDate ? null : day)} style={{
                      padding: 6, border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 12,
                      ...
                      minWidth: 0, width: '100%', aspectRatio: '1'
                    }}>
                      {day}
                      {hasEvent && <div style={{ position: 'absolute', bottom: 2, left: '50%', transform: 'translateX(-50%)', width: 3, height: 3, borderRadius: '50%', background: '#D4AF37' }} />}
                    </button>
                  );
                })}
              </div>
